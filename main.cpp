...
#include "raytracer/rt_photon_map.h"
...
const float PHOTON_RADIUS = 0.35;
...
Scene scene = Scene();
PhotonMap photon_map = PhotonMap();
...
#ifdef USE_PHOTON_MAP
//		light = photon_map.lookup(V, 0.6);
		light = photon_map.radiance_estimate(result->position, *ray->getDirection(), result->normal, PHOTON_ESTIMATE, PHOTON_RADIUS);
#endif
		*color = shadow * light.array() * ((1.0-ref-alpha)*diff_color + ref*ref_color + alpha*refraction_color).array();// + specular*spec_color;
	}
}
...
#ifdef USE_PHOTON_MAP
	photon_map.generate(&scene, PHOTON_COUNT);
	fprintf(stderr, "Photon Map generated\n");
#endif
